...
let export_constructor i ty =
  let ctxt,t = Term.decompose_prod_assum ty in
  let (_,args,_) = Context.fold_rel_context (fun (_,odecl,ty) (subs,args,i) ->
    match odecl with
    | None -> (Constr.mkRel i :: subs, export_arg ty i :: args, i+1)
 ...
 *)
let export_inductive env ((mind,i as ind),u as pind) oib =
  ...
  let ac = Array.mapi export_constructor typs in
...
let export_mind env ((mind,i as ind),u as pind) =
  ...
  let bodies = Array.mapi (fun i -> export_inductive env ((mind,i),u)) mib.mind_packets in
